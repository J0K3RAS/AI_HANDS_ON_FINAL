...
# Οδηγίες εκτέλεσης
## Βασική Χρήση
Η εφαρμογή είναι σχεδιασμένη να τρέχει καλώντας το αρχείο `main.py`, το οποίο παρέχει command line arguments. Αρχικά μπορείτε να κάνετε κάποια διαγράμματα τα οποία αποθηκεύονται στον φάκελο `plots/` δίνοντας το αντίστοιχο flag:
```bash
python main.py --plot
```

Στην συνέχεια, η εργασία αυτή υλοποιεί ένα fully connected νευρωνικό, για την πρόβλεψη της τιμής του αεροπορικού εισιτηρίου. Μπορείτε να εκπαιδεύσετε το εν λόγω μοντέλο με χρήση της εντολής:
```bash
python main.py --nn
```

To μοντέλο αποθηκεύεται στην διαδρομή `models/nnreg.xz`. Επίσης δίνεται η δυνατότητα για tuning ορισμένων υπερπαραμέτρων, δίνοντας συμπληρωματικά το flag --tune, δηλαδή: 
```bash
python main.py --nn --tune
```

Σε αυτή την περπίπτωση, το τελικό μοντέλο αποθηκεύεται ως `models/hp_nnreg.xz`.
...
Κλείνοντας αποφασίζουμε ότι το τελικό μοντέλο που απαντάει στα προαιρετικά ερωτήματα της εργασίας να είναι ένα hybrid μοντέλο, όπου το νευρωνικό δίκτυο κάνει τις προβλέψεις και το xgboost προσπαθεί να διορθώσει την εν λόγω πρόβλεψη. Μπορείτε να το εκπαιδεύσετε ή να κάνετε hyperparameters tuning αξιοποιώντας το --hybrid (--tune) flag. 
```bash
python main.py --hybrid (--tune)
```

Tο μοντέλο αποθηκεύεται ως `models/hybridreg.xz` και `models/hp_hybridreg.xz` αντίστοιχα.

## Endpoint (uvicorn)
Το script `app.py` υλοποιεί τα endpoints της προηγούμενης εργασίας, και έχει προστεθεί η δυνατότητα για αλληλεπίδραση με το τελικό hybrid μοντέλο:
```bash
uvicorn app:app --host 0.0.0.0 --port 8000 --reload
```

Μπορείτε να ζητήσετε από το μοντέλο να σας κάνει μία πρόβλεψη, όπως παρουσιάζεται ακολούθως:
```python
import requests
data = {
 'Days_left': 45,
 'Duration_in_hours': 2.3342357483204217,
...
Τρέχουμε το main.py script που βρίσκεται στο root folder του πρότζεκτ. Από την κονσόλα, παρατηρούμε τα εξής:
...
- Τα δεδομένα έχουν 625 χιλιάδες εγγραφές στις οποίες λείπει η τιμή της μεταβλητής απόκρισης `trip_duration` και κατ' επέκταση η αντίστοιχη ημερομηνία άφιξης `dropoff_datetime`. Η απουσία και των δύο είναι δικαιολογημένη, καθώς η διάρκεια της διαδρομής προκύπτει ως αφαίρεση της ημερομηνίας αφίξης με την ημερομηνία εκκίνησης.  Ο λόγος που απουσιάζουν είναι λόγω της φύσης του διαγωνισμού από τον οποίο προέκυψαν τα δεδομένα, όπου τα "test" δεδομένα δεν έχουν τιμή στην μεταβλητή απόκριση, προκειμένου οι χρήστες να κάνουν submit στο kaggle τα αντίστοιχα predictions και να βαθμολογηθούν, Επομένως, για τους σκοπούς αυτής της άσκησης, θα ασχοληθούμε μόνο με τα train δεδομένα, όπου γενικά δεν υπάρχουν missing values σε κάποιο άλλο feature.
```
Nan values:
id                         0
vendor_id                  0
...
Τα θηκοδιάγραμματα φαίνεται να είναι αρκετά φυσιολογικά ενώ υπάρχουν ποιοτικές διαφορές στις κατανομές των inliners και outliers, παραμένουν κοντά τα θηκοδιαγράμματα των εν λόγω κατηγοριών.
...
```
Το εν λόγω script εφαρμόζει στο train dataset στην Select k best τεχνική για k=6 σε κάθε ένα από τα 4 folds του cross validation, και επιστρέφει τις 6 μεταβλητές με την μεγαλύτερη συχνότητα εμφάνισης στα 4 folds. Επιπροσθέτως, σε κάθε fold εκπαιδεύεται το xgboost μοντέλο και καταγράφεται η απόδοσή του (mean squared error) του validation set. Στην κονσόλα εμφανίζονται τα ακόλουθα αποτέσματα:
...
```
Παρατηρούμε στις 6 καλύτερες περιέχονται 2 από τις μεταβλητές που προστέθησαμε, και όλες όσες σχετίζονται με γεωγραφικές συντεταγμένες. Σε αυτή την περίπτωση, το μοντέλο με τις έξι μεταβλητές καταφέρνει να έχει καλύτερο μέσο τετραγωνικό σφάλμα καλύτερο από το baseline μοντέλο. Στην συνέχεια, το script κατασκευάζει το tree-based feature importance του xgboost μοντέλου:
...
Με βάση τα παραπάνω, <ins>στα επόμενα βήματα της εργασίας αποφασίζουμε να πετάξουμε τις μεταβλητές `store_and_fwd_flag`,  `store_and_fwd_flag`  και `vendor_id`</ins>
...
```
εφαρμόζοντας PCA. Ο κώδικας θα πραγματοποιήσει 4-fold Cross Validation, όπου στα train δεδομένα θα γίνεται fit_transform Standard Scaler και στην συνέχεια PCA, ενώ σκέτο transform στα validation data.  Ακολούθως, τα pca components που προκύπτουν γίνονται concatenate στις κατηγορικές μεταβλητές:
```python
[
    'weekend', 'workhours'
]
...
```
Παρατηρούμε ότι τα πρώτα 4 κύρια συστατικά εξηγούν τον 75% της συνολικής διασποράς. Επιπλέον, από τα μοντέλα που προσαρμόσαμε, είναι φανερό ότι αν θέλαμε να πετύχουμε επίδοση καλύτερη από το baseline μοντέλο, θα έπρεπε να διατηρήσουμε τουλάχιστον 6 κύρια συστατικά. Παρόλα αυτά, το μοντέλο που έχει διατηρήσει και τις 8 συνιστώσες, έχει σημνατικά βελτιωμένη επίδοση σε σχέση με όσα μοντέλα έχουμε εξετάσει μέχρι τώρα. 
...
```
Το μοντέλο παρουσιάζει σημαντικά καλύτερη επίδοση στο train set σε σχέση με το baseline. Ελαφρώς βελτιωμένες είναι και οι μετρικές στα validation & test datasets. Υπάρχει μια μικρή ένδειξη ότι αρχίζει να γίνεται overfit, αλλά από την στιγμή πού όλες οι μετρικές είναι βελτιωμένες, μπορούμε να το αποδεχτούμε.

# Machine Learning model
Ως machine learning model, επιλέγουμε το tabnet model από το πακέτο [pytorch-tabnet](https://github.com/dreamquark-ai/tabnet) , με baseline παραμέτρους (defaults):
```python
{
	 "n_d": 8,
	"gamma": 1.3,
...
{'n_d': 32.0, 'gamma': 1.3, 'n_shared': 4.0, 'lambda_sparse': 0.0001}
