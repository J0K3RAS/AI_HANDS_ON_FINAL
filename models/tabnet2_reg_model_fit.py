...
Example Usage:
1) Train the nn regression model for fare price prediction
    python nn_reg_model_fit.py
        --data-file <Path to train .csv>
        --model-file <Path to old model>
"""
import argparse
import warnings
from datetime import datetime
from models.tabnet2_reg import NNModel
from models.templates.ai_train_template import Trainer

warnings.filterwarnings('ignore')

class NNModelTrain(Trainer):
    """
        Implements the Trainer abstract class, for
        FC Neural Network regression model.
    """
    def __init__(self, train_file, test_file, model_file):
        """
            Constructor of class NNModelTrain.

            :param train_file: str, filepath of train .csv
        """
        super().__init__(train_file, test_file, model_file)
        self.ai_model = NNModel()
    ...
    def load_model(self):
        ...
        other = NNModelTrain(self._train_file, self._test_file, self.model_file)
